...
function train(id) {
    ...
    $.post(`https://www.torn.com/companies.php?rfcv=${getRFC()}`, {
        step: "trainemp2",
        ID: id
    }, function (response) {
...
function insert() {
    ...
    if ($(".employee-list > li").length > 0 && $('a[href^="?step=trainemp"]').length > 0) {
        $(".employee-list > li").each(function () {
            let userid = $(this).attr("data-user");
            $(".days", $(this)).html(`<input type="text" userid="${userid}" class="trainOrder">`);
            if (trains[userid] && trains[userid] > 0) {
                $(".train", $(this)).html(`<button class="torn-btn" id="train${userid}">T</button>`);
        ...
        $(".submit-changes").append(`<button id="updateTrains" class="torn-btn">Update</button>`);
